    ...
    where TSlot : ItemSlot
{
    ...
    protected override void Refresh()
    {
        ...
        for (int idx = 0; idx < slots.Length; idx++)
        {
            ...
            else
            {
                ...
                {
                    if (_onSpawnEffect != null) SpawnViewWithEffect(slot, instance);
    ...
    public Vector3 GetItemWorldPos(int idx) => GetSlotWorldPos(idx) + (Vector3)itemOffset;
    protected TSlot GetSlotAtIndex(int idx) => (TSlot)slots[idx];
    public TView GetViewAt(int idx) => GetViewFromSlot(GetSlotAtIndex(idx));
    ...
    {
        var effectHandled = _onSpawnEffect.Invoke(instance, slot.Root.position, () =>
        {
    ...
    {
        GetViewFromSlot(GetSlotAtIndex(idx))?.PlayHoverAnimation();
        _onHoverEnter?.Invoke(BenchProvider.Bench[idx], GetItemWorldPos(idx));
    }
